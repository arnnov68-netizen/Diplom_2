...
class TestCreateOrder:
    ...
    @allure.title("Создание заказа с корректными ингредиентами")
    @allure.description("Проверка создания заказа с корректными ID ингредиентов")
    @allure.severity(allure.severity_level.CRITICAL)
    def test_create_order_with_ingredients(self, auth_token, ingredient_ids):
        ...
        data = response.json()
        with allure.step("Проверка наличия заказа"):
            assert data['success'] is True
            assert 'order' in data
            assert 'number' in data['order']
            assert data['order']['number'] > 0
